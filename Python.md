...
* В хэш-таблице _dict_ каждому элементу соотвествует ячейка, содержащая два поля: ссылку на ключ и ссылку на значение элемента
...
* События входа и выхода из блока определены методами `__enter__` и `__exit__`
    ...
    * `__exit__` должен содержать следующие аргументы `(exc_class, exc_instance, traceback)` так как он будет возращать их в случае выхода из блока по причине исключения
