...
* Чтобы разобраться разобраться в порядке исполнения всех подмешенных `trait` выпишем все зависимости класса `B`
    * Начинаем разбирать зависимости класса `В` слева направо, а провалившись вовнутрь суперкласса выписываем его зависимости справа налево (и так рекурсивно)
       ...
       3. trait S -> trait H
    * Теперь мы собираем все зависимости начиная с 1ой строки справа налево и игнорируя дубли
        * Получаем X, R, A, H, T, S - именно такая последовательность выполнения `trait` подмешенных в класс `B`, что соответствует результату представленному выше
### Case class
---
## ФП
