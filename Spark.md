...
2. Код, который нужно исполнить Spark передается на **Driver**, где
    * Строится план запроса(ов) при помощи **Catalyst**
...
4. **Executors** передают информацию о процессе вычислений на **Driver**
5. Если **Driver** завершает работу, приложение закончено
#### Дополнительно
...
* Хинты (подсказки) - дополнительная параметризация джоинов, которую может задать пользователь, чтобы оптимизировать расчеты
    * Подсказки по разделению - эти хинты дают пользователям возможность настроить производительность и контролировать количество выходных файлов в Spark SQL
    ...
    * Подсказки по соединению - позволяют пользователям предлагать движку Spark SQL оптимальную стратегию соединения (но не гарантируют, что будет именно так)
        * **BROADCAST** — широковещательное соединение. Сторона соединения с подсказкой будет транслироваться независимо от значения параметра _autoBroadcastJoinThreshold_
