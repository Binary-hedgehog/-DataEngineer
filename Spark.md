...
* Справочная информация
    ...
    * Equi Join - эквивалентные джойны включают одно условие равенства, либо несколько, которые должны выполняться одновременно `(A.x == B.x)` или `((A.x == B.x) и (A.y == B.y))`
    * Non-Equi Join - неэквивалентные джойны не подразумевают условий равенства
        * По факту - это когда в условиях есть что-то кроме равенства
* Виды Join
  * Left/Right (Left/Right Outer) - оставляет все из левой таблицы, добавляет туда то, что нашел в правой. Несцепленные строки из левой заполняет null в колонках правой
...
* Механизмы Join
  * **Broadcast Hash Join**
    ...
    * Из плюсов - отсутствие операций `sort`, `merge` 
    * Из минусов - требуют много места в памяти
  ...
  * **Shuffle Hash Join**
    * Две таблицы выравниваются (перемешиваются) по схеме партиционирования (либо перемешиваются до похожести)
  ...
  * **Sort Merge Join** (он же Shuffle Sort Merge Join)
    * Состоит из трех фаз
        ...
        * Sort
        * Merge
    * Merge по сортированным данным проходит быстрее, чем обычный, так как после схождения ключа все что было выше не рассматривается
    * Такую стратегию можно использовать для двух очень больших датасетов, которые с трудом помещаются в памяти
    * Из плюсов - меньшнее потребление оперативки чем у **Shuffle Hash Join**
    * Из минусов 
        * За плюс выше обычно приходится платить скоростью фазы Merge
        * Только **Equi Join**
        * Ключи должны поддаваться сортировке
  * **Cartesian Join** - Декартово соединение ------ кто ты блять такей нахуй??????????
    * Используется исключительно для **Inner Join**
    * Поддержка **Non-Equi Join**
  * **Broadcast Nested Loop Join**
    * Входной набор транслируется на всех
    * А дальше целиком циклом по всем ключам
    * Плюсы - поддержка **Non-Equi Join**
    * Минусы
        * Много времени
        * Много памяти
