...
* **Catalyst** — оптимизатор запросов, который рабоатет на Driver
    * Поддерживает как SQL‑запросы, так и DataFrame API, оптимизатор **Catalyst** использует расширенные функции языка программирования для построения более оптимальных запросов
